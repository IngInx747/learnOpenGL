...
void Cube :: Update(glm::vec3 camPos) {
	...
	for (auto it=distdict.rbegin(); it!=distdict.rend(); it++) {
		...
		for (int i=0; i<4; i++) {
			...
			v.x = cube_vertices[face_id * 4 + i][0];
			v.y = cube_vertices[face_id * 4 + i][1];
			v.z = cube_vertices[face_id * 4 + i][2];
			vertex.position = v;

			v.x = cube_vertices[face_id * 4 + i][3];
			v.y = cube_vertices[face_id * 4 + i][4];
			v.z = cube_vertices[face_id * 4 + i][5];
			vertex.normal = v;

			u.x = cube_vertices[face_id * 4 + i][6];
			u.y = cube_vertices[face_id * 4 + i][7];
			vertex.texCoords = u;
