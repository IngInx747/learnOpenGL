...
vec4 CalcReflection(vec3 normal, vec3 viewDir,
	sampler2D ambient);
...
void main() {
	...
	vec4 reflectColor = CalcReflection(normal, viewDir, uMaterial.texture_ambient1);
	...
	// Result
	resultColor = directionalLightColor + spotLightColor + reflectColor;
...
vec4 CalcReflection(vec3 normal, vec3 viewDir,
	...
	vec3 reflectTexture = texture(ambient, TexCoords).rgb;
	return vec4(reflectColor * reflectTexture, 1.0);
}
